...
public class Mote {
    ...
    @Expose private int KeepAliveTime;
    //@Expose private double FilledQueue;
    @Expose private int QueueLoss;
    @Expose private int QueueSize;
    @Expose private int MAX_QUEUE_SIZE;
    
    public Mote() { }
    
    
    public Mote(Integer moteid, int load, Double battery,Integer parents, int dataProbability, int queueSize, int queueLoss, int keepAliveTime, int MAX_QUEUE_SIZE, List<Link> links) {
		this.MoteID = moteid;
		this.Load = load;
    ...
    public Object clone() {
    	...
    	return new Mote(MoteID, Load, BatteryRemaining, NumParents, DataProbability,QueueSize,QueueLoss,KeepAliveTime,MAX_QUEUE_SIZE,linksCopy);
    }
}
